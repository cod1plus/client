// Auto-upload de demos CoD1.
//
// Workflow :
//   1. Au demarrage, on detecte le dossier `main/demos/` du jeu.
//   2. Thread background : scan recurring tous les N sec.
//   3. Pour chaque `.dm_1` trouve qui n'a pas le suffixe `.uploaded` :
//      a. Verifie que le fichier est "stable" (taille n'a pas change depuis
//         2 cycles de poll) -> evite d'upload pendant que l'engine ecrit.
//      b. HTTP POST le fichier en body (multipart/form-data fallback inutile).
//      c. Toast Windows : "Demo uploading...", "Upload success", "Upload failed".
//      d. Sur success : delete ou rename en `.uploaded` selon config.
//
// Limitation V1 : on n'a pas de hook engine pour declencher le record auto.
// L'utilisateur doit faire `/record demoX` manuellement (ou via un script GSC
// cote serveur). Phase 2 ajoutera le hook auto-record.
...
// Event manuel-reset utilise pour reveiller le thread d'upload entre 2 polls
// quand un evenement externe (quit, disconnect) survient.
HANDLE g_wake_event = NULL;

// Resoud le dossier des demos via le chemin du jeu (CoDMP.exe).
// Renvoie via out_path. False si introuvable.
...
// HTTP POST d'un fichier en body. Renvoie true si HTTP 200/2xx.
bool http_post_file(const char* url, const char* file_path,
    ...
    // Parse URL pour extraire host + path
    char host[256] = {0};
    char path[512] = "/";
    bool https = false;
    ...
    // Envoie en chunks via HttpSendRequestEx (pour les gros fichiers)
...
// Itere les .dm_1 dans le dossier. Pour chacun :
//   - Skip si fichier .uploaded a cote OU si le nom finit deja par .uploaded
//   - Verifie stabilite (size identique au cycle precedent)
//   - Upload
//   - Sur success : delete ou rename
void scan_and_upload(const char* demos_dir,
    ...
    do {
        ...
        // Skip si un .uploaded marker existe
        ...
        // Verifie stabilite : on a vu ce fichier au cycle precedent ET la
        // taille n'a pas change. Sinon on attend le prochain poll.
        ...
        if (it == size_cache.end() || it->second != current_size) {
            ...
            continue; // attend le prochain cycle
        }
        ...
        if (ok) {
            ...
            if (g_demo_upload_config.delete_after_upload) {
                ...
                // Cree un marker pour ne pas re-upload
            ...
            // Ne touche pas au fichier - on retentera au prochain poll
        }
...
DWORD WINAPI demo_upload_thread(LPVOID) {
    ...
    // Cache des tailles pour la detection de stabilite entre 2 polls.
    std::unordered_map<std::string, long> size_cache;
    ...
    // Premier scan immediat puis polling avec reveil sur event
    while (true) {
        scan_and_upload(demos_dir, size_cache);
        DWORD wait_ms = (DWORD)interval * 1000;
        // Si l'event est signale (trigger_now appele), on reveille tot.
        // Sinon on attend jusqu'au timeout du poll normal.
        if (g_wake_event) {
            WaitForSingleObject(g_wake_event, wait_ms);
            // Manual reset event - on remet a zero pour le prochain trigger
